    ...
    public static Timestamp parseTimestamp(String input) throws IllegalArgumentException {
        ...
        if (pos >= 0) {
            ...
            if (micro.isEmpty() || micro.length() > 6) {
                throw new IllegalArgumentException("Error occurred during parsing microseconds");
            }
            micro += "000000".substring(0, 6 - micro.length());
            try {
                microseconds = Integer.parseInt(micro);
            } catch (NumberFormatException ex) {
                throw new IllegalArgumentException("Error occurred during parsing microseconds", ex);
            }
        }
    ...
    public Timestamp read(JsonReader in) throws IOException {
        ...
        if (jsonToken == JsonToken.NULL) {
            ...
            try {
                ...
                throw new IOException("Unparseable timestamp", e);
            }
        }
    }
}
